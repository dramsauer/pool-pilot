...
RUN apt-get update && apt-get install -y --no-install-recommends     git curl ca-certificates make &&     rm -rf /var/lib/apt/lists/*
...
CMD ["streamlit", "run", "Wasserrechner.py", "--server.address=0.0.0.0", "--server.port=8501"]
